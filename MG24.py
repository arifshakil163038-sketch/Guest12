...
BASE_FOLDER = os.path.join(CURRENT_DIR, "MG24-ERA")
...
hex_data = "8J+agCBQUkVNSVVNIEFDQ09VTlQgR0VORVJBVE9SIPCfkqsgQnkgTUcyNCBHQU1FUiA=IHwgTm90IEZvciBTYWxlIPCfkas="
client_data = base64.b64decode(hex_data).decode('utf-8')
GARENA = "QllfTUcyNF9HQU1FUg=="
...
def save_rare_account(account_data, rarity_type, reason, rarity_score, is_ghost=False):
    try:
        ...
        rare_entry = {
            ...
            'region': "MG24" if is_ghost else account_data.get('region', 'UNKNOWN'),
...
def save_couples_account(account1, account2, reason, is_ghost=False):
    try:
        ...
        couples_entry = {
            ...
            'reason': reason,
            'region': "MĢ24_GÀMER" if is_ghost else account1.get('region', 'UNKNOWN'),
...
def save_jwt_token(account_data, jwt_token, region, is_ghost=False):
    try:
        ...
        token_entry = {
            ...
            'region': "MĢ24_GÀMER" if is_ghost else region,
            'thread_id': account_data.get('thread_id', 'N/A')
        }
    ...
    while True:
        try:
            ...
            if choice == "00":
                ...
                print(f"\n{get_random_color()}{Colors.BRIGHT}👋 Thank you for using MĢ24_GÀMER Generator!{Colors.RESET}")
                sys.exit(0)
            elif choice.isdigit():
...
def about_section():
    ...
    print(f"{get_random_color()}{Colors.BRIGHT}ℹ️  About MĢ24_GÀMER Account Generator{Colors.RESET}")
...
def main_menu():
    while True:
        ...
        print(f"{get_random_color()}{Colors.BRIGHT}🎮 Welcome to MĢ24_GÀMER Account Generator{Colors.RESET}")
